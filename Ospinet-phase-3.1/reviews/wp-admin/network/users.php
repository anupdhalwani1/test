...
<p>
<?php _e( 'Transfer or delete posts before deleting users.' ); ?>
</p>
<form action="users.php?action=dodelete" method="post">
	...
	<fieldset>
		<p>
			<legend>
			<?php printf( __( "What should be done with posts owned by <em>%s</em>?" ), $delete_user->user_login ); ?>
			</legend>
		</p>
		<?php
		...
		<ul style="list-style: none;">
			...
			<li><label><input type="radio" id="delete_option0"
					...
					value="delete" checked="checked" /> <?php _e( 'Delete all posts.' ); ?>
			</label></li>
			<li><label><input type="radio" id="delete_option1"
					name="delete[<?php echo $details->userblog_id . '][' . $delete_user->ID ?>]"
					value="reassign" /> <?php echo __( 'Attribute all posts to:' ) . '</label>' . $user_dropdown; ?>
...
<?php screen_icon(); ?>
	...
	if ( current_user_can( 'create_users') ) : ?>
		<a href="<?php echo network_admin_url('user-new.php'); ?>"
			class="add-new-h2"><?php echo esc_html_x( 'Add New', 'user' ); ?> </a>
			<?php
			endif;
	...
	<form id="form-user-list" action='users.php?action=allusers'
		method='post'>
