...
<form id="search-themes" method="get" action="">
	...
	<label class="screen-reader-text" for="typeselector"><?php _e('Type of search'); ?>
	</label> <select name="type" id="typeselector">
		...
		<?php _ex('Tag', 'Theme Installer'); ?>
		</option>
	</select> <label class="screen-reader-text" for="s"><?php
	switch ( $type ) {
		case 'term':
	...
	<?php endif; ?>
	<input type="search" name="s" id="s" size="30"
		value="<?php echo esc_attr($term) ?>" autofocus="autofocus" />
...
<?php _e( 'Find a theme based on specific features.' ); ?>
</p>
...
<form method="get" action="">
	...
	<li><input type="checkbox" name="features[]"
		...
		value="<?php echo $feature; ?>" /> <label
		for="feature-id-<?php echo $feature; ?>"><?php echo $feature_name; ?>
	</label>
	</li>
...
function install_themes_upload($page = 1) {
	?>
<h4>
<?php _e('Install a theme in .zip format'); ?>
</h4>
<p class="install-help">
<?php _e('If you have a theme in a .zip format, you may install it by uploading it here.'); ?>
</p>
