...
<?php if ( !$api_key ) : ?>
	...
	<?php else: ?>
	<h2 class="ak-header">
	<?php printf( __( 'Akismet <a href="%s" class="add-new-h2">Stats</a>' ), $stats_link ); ?>
	</h2>
	<?php endif; ?>
	<div class="no-key <?php echo $show_key_form ? 'hidden' : '';?>">
		<p>
		<?php _e('Akismet eliminates the comment and trackback spam you get on your site. To use Akismet you may need to sign up for an API key. Click the button below to get started.'); ?>
		</p>
		<form name="akismet_activate" action="https://akismet.com/get/"
			method="POST">
			<input type="hidden" name="return" value="1" /> <input type="hidden"
				name="jetpack"
				value="<?php echo (string) class_exists( 'Jetpack' );?>" /> <input
				type="hidden" name="user"
				value="<?php echo esc_attr( $current_user->user_login );?>" /> <input
				type="submit" class="button button-primary"
				value="<?php esc_attr_e( 'Create a new Akismet Key' ); ?>" />
		</form>
		<br /> <a href="<?php echo $config_link;?>"><?php _e('I already have a key'); ?>
		</a>
	</div>
	...
	<?php if ( !empty($_POST['submit'] ) && $saved_ok ) : ?>
		...
		<div id="message" class="updated fade">
			<p>
			<?php _e('<strong>Sign up success!</strong> Please check your email for your Akismet API Key and enter it below.') ?>
			</p>
		</div>
		<?php endif; ?>
		<?php foreach( $ms as $m ) : ?>
		<div class="<?php echo $messages[$m]['class']; ?>">
			<p>
				<strong><?php echo $messages[$m]['text']; ?> </strong>
			</p>
		</div>
		<?php endforeach; ?>
		<form action="" method="post" id="akismet-conf">
			<table class="form-table">
				<tbody>
					<tr>
						...
						<td><input id="key" name="key" type="text" size="15"
							maxlength="12"
							value="<?php echo esc_attr( get_option('wordpress_api_key') ); ?>"
							class="regular-text code <?php echo $key_status;?>">
							<div class="under-input key-status <?php echo $key_status;?>">
							<?php echo isset( $key_status_strings[ $key_status ] ) ? $key_status_strings[ $key_status ] : '';?>
							</div>
							<p class="need-key description">
							<?php printf( __('You must enter a valid Akismet API key here. If you need an API key, you can <a href="%s">create one here</a>'), '#' );?>
							</p>
						</td>
					</tr>
					<?php if ( $api_key ):?>
						...
						<td>
							<fieldset>
								...
								<label for="akismet_discard_month"
									title="<?php esc_attr_e( 'Auto-detete old spam' ); ?>"><input
									name="akismet_discard_month" id="akismet_discard_month"
									value="true" type="checkbox"
									<?php echo get_option('akismet_discard_month') == 'true' ? 'checked="checked"':''; ?>>
									<span><?php _e('Auto-delete spam submitted on posts more than a month old.'); ?>
								</span> </label><br> <label
									for="akismet_show_user_comments_approved"
									title="<?php esc_attr_e( 'Show approved comments' ); ?>"><input
									name="akismet_show_user_comments_approved"
									id="akismet_show_user_comments_approved" value="true"
									type="checkbox"
									<?php echo get_option('akismet_show_user_comments_approved') == 'true' ? 'checked="checked"':''; ?>>
									<span><?php _e('Show the number of comments you\'ve approved beside each comment author.'); ?>
								</span> </label>
							</fieldset>
						</td>
					</tr>
			...
			<p class="submit">
				<input type="submit" name="submit" id="submit"
					class="button button-primary" value="<?php _e('Save Changes');?>">
			</p>
		</form>

		<?php if ( $api_key ) : ?>
		<h3>
		<?php _e('Server Connectivity'); ?>
		</h3>
			...
			<table class="form-table">
				<tbody>
					<tr>
						...
						<td><?php if ( !function_exists('fsockopen') || !function_exists('gethostbynamel') ) : ?>
							<p class="key-status failed">
							<?php _e('Network functions are disabled.'); ?>
							</p>
							<p class="description">
							<?php echo sprintf( __('Your web host or server administrator has disabled PHP\'s <code>fsockopen</code> or <code>gethostbynamel</code> functions.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet\'s system requirements</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?>
							</p> <?php else :
							...
							<p class="key-status some">
							<?php _e('Unable to reach some Akismet servers.'); ?>
							</p>
							<p class="description">
							<?php echo sprintf( __('A network problem or firewall is blocking some connections from your web server to Akismet.com.  Akismet is working but this may cause problems during times of network congestion.  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet and firewalls</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?>
							</p> <?php } elseif ( $fail_count > 0 ) { // all connections fail ?>
							<p class="key-status failed">
							<?php _e('Unable to reach any Akismet servers.'); ?>
							</p>
							<p class="description">
							<?php echo sprintf( __('A network problem or firewall is blocking all connections from your web server to Akismet.com.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet and firewalls</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?>
							</p> <?php } else { // all connections work ?>
							<p class="key-status valid">
							<?php  _e('All Akismet servers are available.'); ?>
							</p>
							<p class="description">
							<?php _e('Akismet is working correctly.  All servers are accessible.'); ?>
							</p> <?php }
							} else { //can't connect to any server ?>
							<p class="key-status failed">
							<?php _e('Unable to find Akismet servers.'); ?>
							</p>
							<p class="description">
							...
							<table class="network-status">
								...
								foreach ( $servers as $ip => $status ) : ?>
									<tr>
										...
										<td
											class="key-status <?php echo $status ? 'valid' : 'failed'; ?>"><?php echo $status ? __('Accessible') : __('Re-trying'); ?>
										</td>
									</tr>
									<?php endforeach; ?>
								</tbody>
							</table> <br /> <input type="submit" name="check" id="submit"
							class="button" style="margin-left: 13.3em;"
							value="<?php _e('Check Network Status');?>">
						</td>
					</tr>
					<?php endif; ?>
					<tr valign="top">
						...
						<td>
							<p>
								<strong><?php echo get_option('akismet_connectivity_time') ? sprintf( __('%s Ago'), ucwords( human_time_diff( get_option('akismet_connectivity_time') ) ) ) : __( 'Not yet' ); ?>
								</strong>
							</p>
							<p class="description">
							<?php printf( __('You can confirm that Akismet.com is up by <a href="%s" target="_blank">clicking here</a>.'), 'http://status.automattic.com/9931/136079/Akismet-API' ); ?>
							</p>
						</td>
...
if ( !$api_key ) :?>
	<div id="akismet-warning" class="updated fade">
		<p>
			<strong><?php _e('Akismet is almost ready.');?> </strong>
			<?php printf( __( 'You must <a href="%1$s">enter your Akismet API key</a> for it to work.' ), $config_link );?>
		</p>
	</div>
	<?php
	else :?>
	<iframe
		src="<?php echo esc_url( sprintf( '%s://akismet.com/web/1.0/user-stats.php?blog=%s&api_key=%s', is_ssl()?'https':'http', $blog, $api_key ) ); ?>"
...
<div class='error'>
	<p>
		<strong><?php _e( 'Akismet Error Code');?>: <?php echo $alert['code']; ?>
		</strong>
	</p>
	<p>
	<?php esc_html_e( $alert['msg'] ); ?>
	</p>
	<p>
	<?php //FIXME: need to revert this to using __() in next version
